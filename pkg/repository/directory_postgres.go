...
func (r *DirectoryPostgres) AddDirectory(userId int, objectId int, nameDirectory string) error {
	query := fmt.Sprintf("INSERT INTO %s (name, server_name, create_date, user_id, parent_id, type_object_id) values ($1, $2, $3, $4, $5, $6)", objectsTable)
	_, err := r.db.Exec(query, nameDirectory, nameDirectory, time.Now(), userId, objectId, 2)
	if err != nil {
		return err
	}
	return nil
